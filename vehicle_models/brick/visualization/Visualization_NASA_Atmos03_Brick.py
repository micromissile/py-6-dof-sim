...
if __name__ == '__main__':  # NOTE: This is REQUIRED on Windows!
    ...
    data_03_pysim  = np.load('./case_studies/verification/Lead_Video/Atmos_03_Python_6DOF.npy')
    
    # Get time and other variables
    t_s   = data_03_pysim[:,0]; nt_s = t_s.size
    ...
    while True:
        ...
        v_body_u_parent     = data_03_pysim[i,1]*3.28 # (converts m/s to ft/s)
        v_body_v_parent     = data_03_pysim[i,2]*3.28
        v_body_w_parent     = data_03_pysim[i,3]*3.28
        phi_rad_parent      = data_03_pysim[i,7]
        theta_rad_parent    = data_03_pysim[i,8]
        psi_rad_parent      = data_03_pysim[i,9]
        alt_m_parent        = data_03_pysim[i,13]
        alpha_rad_parent    = data_03_pysim[i,17]
        beta_rad_parent     = data_03_pysim[i,18]
